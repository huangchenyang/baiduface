...
public class GazeFaceDetectActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void init() {
        ...
        rgbAndNirScoreThreshold = SingleBaseConfig.getBaseConfig().getRgbAndNirThreshold();
        // 模态切换光线阈值
        lightThreshold = SingleBaseConfig.getBaseConfig().getCameraLightThreshold();
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.qc_save) {
            ...
            SingleBaseConfig.getBaseConfig().setLiveThreshold(
                    Float.valueOf(thLiveEtThreshold.getText().toString()));
            SingleBaseConfig.getBaseConfig().setIdThreshold(
                    Float.valueOf(thIDEtThreshold.getText().toString()));
            SingleBaseConfig.getBaseConfig().setRgbAndNirThreshold(
                    Float.valueOf(mixtureIDEtThreshold.getText().toString()));
            ...
            if (liveInitValue > zero && liveInitValue <= HUNDERED) {
                liveInitValue = liveInitValue - 0.1f;
                thLiveEtThreshold.setText(liveInitValue + "");
            }
            ...
            if (rgbAndNirScoreThreshold > zero && rgbAndNirScoreThreshold <= HUNDERED) {
                rgbAndNirScoreThreshold = rgbAndNirScoreThreshold - 0.1f;
                mixtureIDEtThreshold.setText(rgbAndNirScoreThreshold + "");
            }
